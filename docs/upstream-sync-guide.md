...
├── korea-weather/          ├── skills/weather/__init__.py   ← Click 명령어 구현
│   └── SKILL.md            ├── skills/weather/manifest.yaml ← 메타데이터
│   (프롬프트 정의)         └── proxy.py → /v1/korea-weather/forecast
```
...
### 2. 신규 스킬 감지
```bash
# k-skill에 있는 스킬 vs k-cli에 래핑된 스킬 비교
diff <(cd ~/nas_1tb/dev/k-skill && find . -maxdepth 1 -name '*-search' -o -name 'k-*' | sort | sed 's|./||') \
...
**기존 도메인에 추가** (예: `life/`에 `emergency-room-beds` 추가):
1. `skills/life/__init__.py`에 `@cli.command()` 추가
2. `skills/life/manifest.yaml`에 스킬 엔트리 추가

**새 도메인 생성** (예: 카테고리가 기존에 없는 경우):
1. `skills/<new-domain>/__init__.py` 생성 (클릭 그룹 + 명령어)
...
## 예시: 신규 스킬 6종 래핑 (2026-05-23 기준 미반영)

| 스킬 | 호출 방식 | 프록시 라우트 | 할당 도메인 |
|------|----------|-------------|-----------|
| emergency-room-beds | 직접 HTTP (E-Gen) | 없음 | `life/` |
| korean-cinema-search | MCP (daiso 패키지) | 없음 | `sports/` |
| kstartup-search | **프록시** | `/v1/kstartup/*` (4개) | 신규 또는 `finance/` |
| local-election-candidate-search | 직접 HTTP (NEC) | 없음 | `life/` |
| ohou-today-deal | 직접 HTTP (오늘의집) | 없음 | `shopping/` |
| sh-notice-search | **프록시** | 없음 (추가 필요?) | `realestate/` |
...
- [ ] `cli.py` 도움말 업데이트 (에이전트 빠른 실행 가이드)
